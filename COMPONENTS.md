...
| 1 | `HeroLibraryDemo` | `hero/HeroLibraryDemo.tsx` + `.css` | React port of the macOS Library window, synced to v0.13.x (2026-06-10). Interactive: **left tabs Transcript / Summary / Chapters** switch real content (markdown summary; green timecode-pill chapters with progress fill); **breadcrumb "Dashboard"** opens a full **Dashboard view** (Stats / Upcoming tabs + "Longest" recent list); **Archive** button swaps the sidebar to the archive list (green active, like the app); **Settings gear** opens the right Settings pane; recent cards + sidebar items return to the meeting. Header = theme-moon, Settings gear, Archive, **admin profile avatar** (blue `--avatar-admin` disc + half-moon glyph). Right column = Recording (video preview + scrubber + per-speaker Timeline) / Settings. Traffic-light dots reveal × / − / fullscreen glyphs on window hover; gentle pointer 3D tilt; moon = radial View-Transition theme flip. **Deliberate deviation:** the app moved theme into Settings, but the header theme-moon is kept as an interactive demo affordance. CSS is a manual port of the app's `styles.css` (no auto-sync). |
